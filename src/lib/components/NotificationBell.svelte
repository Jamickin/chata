...
<div class="relative" bind:this={bellRef}>
	...
	>
		...
		<svg
			...
			class="w-5 h-5 text-slate-300 {unreadCount > 0 ? 'animate-wiggle' : ''}"
			viewBox="0 0 24 24"
			fill="none"
			stroke="currentColor"
			stroke-width="2"
	...
	{#if showNotifications}
		<div
			class="fixed top-16 right-4 md:right-8 w-80 max-h-96 overflow-y-auto bg-slate-800 rounded-lg shadow-xl border border-slate-700 animate-slideDown"
		>
			<div class="p-3 border-b border-slate-700 flex justify-between items-center">
				<h3 class="font-semibold text-white">Notifications</h3>
				{#if unreadCount > 0}
					<button class="text-xs text-blue-400 hover:underline" onclick={markAllAsRead}>
						Mark all as read
					</button>
				{/if}
			</div>
			...
			{#if notifications.length === 0}
				<div class="p-4 text-center text-slate-400">No notifications to display</div>
			{:else}
				<div>
					{#each notifications as notification}
						<div
							class="p-3 border-b border-slate-700 hover:bg-slate-700/50 transition-colors relative {notification.read
								? 'opacity-75'
								: ''}"
						>
							<div class="flex justify-between items-start">
								<h4 class="font-medium ttext-white text-sm">
									{notification.title}
								</h4>
							...
							<p class="text-xs text-slate-500 mt-2">{notification.time}</p>
							{#if !notification.read}
			...
			<div class="p-2 text-center border-t border-slate-700">
				<a href="/toni" class="text-xs text-blue-400 hover:underline">View all notifications</a>
			</div>
		</div>
	{/if}
</div>
